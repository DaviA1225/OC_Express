...
import { duracaoLegivel, horaCurta } from '@/features/agendamentos/agendamento'
import {
  useTerminalJanelas,
  useSalvarJanela,
  useRemoverJanela,
...
function Conteudo({ row, onOpenChange, onSubmit }: { row: Cliente } & Omit<Props, 'row'>) {
  ...
  const jaExistem = new Set(lista.map((j) => j.hora))
  const ineditos = previa.filter((h) => !jaExistem.has(h))
          ...
          <>
            ...
            <div className="space-y-2 rounded-lg border bg-card p-3">
              ...
              <p className="text-[11px] leading-relaxed text-muted-foreground">
                ...
                sistema do terminal, que também atende outras transportadoras.
              </p>
                  ...
                  {lista.map((j) => (
                    ...
                    >
                      ...
                      <Switch
                        ...
                        aria-label={`Ativar horário ${horaCurta(j.hora)}`}
                      />
                      <button
                        type="button"
                        ...
                        aria-label={`Remover horário ${horaCurta(j.hora)}`}
