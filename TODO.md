...
- [x] **UPDATE do parceiro só cancela** — migration `0028_parceiro_update_so_cancela.sql`: a policy `solicitacoes_parceiro_cancel` (0018) deixava o parceiro dar UPDATE em qualquer campo (e forçar qualquer `status`) na própria solicitação `recebida` via API direta — o portal só expunha "Cancelar". Agora o `WITH CHECK` exige `status='cancelada'`: a única transição do parceiro é `recebida → cancelada`. Cancelamento do portal (`update({status:'cancelada'})`) intacto; interno usa `solicitacoes_interno_update`. Pentest +2 cenários (força status / edita campo mantendo recebida → ambos bloqueados). **Pendente:** aplicar a migration no Supabase + rodar o pentest.
